...
void mod_v() {
    ...
    if ((value1)->isVector()) {
        ...
        if (value2->isInteger()){
            ...
            Value* node;
            for (int i = 0; i < size; i++){
                node = new Value(fmodf(scalar, *(value1->vectorValue()->get(i)->realValue())));
                vectorValues->append(node);
            }
        ...
        if (node->isInteger()){
            ...
            for (int i = 0; i < size2; i++){
                node = new Value(fmodf(*(value2->vectorValue()->get(i)->realValue()), *(value1->vectorValue()->get(i)->realValue())));
                vectorValues->append(node);
            }
        ...
        Value* node;
        for (int i = 0; i < size2; i++){
            node = new Value(fmodf(*(value2->vectorValue()->get(i)->realValue()), scalar));
            vectorValues->append(node);
        }
